...
public class Day7Part2 {
    ...
    private static String INPUT2 = "shiny gold bags contain 2 dark red bags.\n" +
    ...
    public static void main(String[] args) {
        ...
        Map<String, List<Bag>> mappings = new HashMap<>();
        for (String rule : rules) {
            String firstBag = getFirstBag(rule);
            List<Bag> otherBags = getOtherBags(rule);
            for (Bag otherBag : otherBags) {
                if (mappings.containsKey(firstBag)) {
                    mappings.get(firstBag).add(otherBag);
                } else {
                    List<Bag> bags = new ArrayList<>();
                    bags.add(otherBag);
                    mappings.put(firstBag, bags);
                }
            }
        }
...
//        }
    ...
    public static int search(Map<String, List<Bag>> mappings, List<Bag> next) {
        if (next == null)
            return 0;
        int cnt = 0;
        for (Bag n : next) {
            cnt += n.getCount() + (n.getCount() * search(mappings, mappings.get(n.getColor())));
        }
        return cnt;
    }
    ...
    private static List<Bag> getOtherBags(String rule) {
        List<Bag> others = new ArrayList<>();
        String[] parts = rule.split(", ");
        if (!parts[0].endsWith("contain no other bags.")) {
            String[] p2 = parts[0].split(" ");
            others.add(new Bag(p2[5] + " " + p2[6], Integer.parseInt(p2[4])));
            for (int i = 1; i < parts.length; i++) {
                String[] p3 = parts[i].split(" ");
                others.add(new Bag(p3[1] + " " + p3[2], Integer.parseInt(p3[0])));
            }
        }
        return others;
    }
