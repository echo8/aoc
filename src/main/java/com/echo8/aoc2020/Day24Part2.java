...
//        System.out.println(coordMap);
        ...
        for (int day = 1; day <= 100; day++) {
            Map<Coord, Boolean> newCoordMap = new HashMap<>();
            for (Coord c : new ArrayList<>(coordMap.keySet())) {
                List<Coord> adjs = getAdjacents(c);
                for (Coord a : adjs) {
                    if (!coordMap.containsKey(a)) {
                        coordMap.put(a, false);
                    }
                }
            }
            for (Map.Entry<Coord, Boolean> entry : coordMap.entrySet()) {
                int adjBlackCount = getAdjacentBlackTileCount(entry.getKey(), coordMap);
                if (entry.getValue()) {
                    if (adjBlackCount == 0 || adjBlackCount > 2) {
                        newCoordMap.put(entry.getKey(), false);
                    } else {
                        newCoordMap.put(entry.getKey(), true);
                    }
                } else {
                    if (adjBlackCount == 2) {
                        newCoordMap.put(entry.getKey(), true);
            ...
            System.out.println("Day " + day + ": " + coordMap.values().stream().filter(v -> v).count());
        }
    }

    private static List<Coord> getAdjacents(Coord c) {
        Coord w = new Coord(c.x-1, c.y);
        Coord e = new Coord(c.x+1, c.y);
        Coord nw = new Coord(c.x, c.y-1);
        Coord ne = new Coord(c.x + 1, c.y - 1);
        Coord sw = new Coord(c.x - 1, c.y + 1);
        Coord se = new Coord(c.x, c.y + 1);
        return Arrays.asList(w, e, nw, ne, sw, se);
    }

    private static int getAdjacentBlackTileCount(Coord c, Map<Coord, Boolean> coordMap) {
        int cnt = 0;
        List<Coord> adjs = getAdjacents(c);
        for (Coord a : adjs) {
            if (coordMap.containsKey(a) && coordMap.get(a)) {
                cnt++;
            }
        }
        return cnt;
