...
public class Day16Part2 {
    private static final String INPUT = "departure location: 27-840 or 860-957\n" +
    ...
    private static final String INPUT2 = "class: 0-1 or 4-19\n" +
            "row: 0-5 or 8-19\n" +
            "seat: 0-13 or 16-19\n" +
            "\n" +
            "your ticket:\n" +
    ...
    private static class Field {
        ...
        public Field(String name, int lower, int upper) {
            this.name = name;
            this.lower = lower;
            this.upper = upper;
        }
    ...
    public static void main(String[] args) {
        TreeSet<Field> setByLower = new TreeSet<>(Comparator.comparingInt((Field f) -> f.lower).thenComparingInt(f -> f.upper));
        TreeSet<Field> setByUpper = new TreeSet<>(Comparator.comparingInt((Field f) -> f.upper).thenComparingInt(f -> f.lower));
        String[] lines = INPUT.split("\n");
        int l = 0;
        Set<String> fieldNames = new HashSet<>();
        while (!lines[l].equals("")) {
            String[] fieldParts = lines[l].split(": ");
            String[] rangeParts = fieldParts[1].split(" or ");
            String[] range1 = rangeParts[0].split("-");
            String[] range2 = rangeParts[1].split("-");
            Field f1 = new Field(fieldParts[0], Integer.parseInt(range1[0]), Integer.parseInt(range1[1]));
            Field f2 = new Field(fieldParts[0], Integer.parseInt(range2[0]), Integer.parseInt(range2[1]));
        ...
        l += 2;
        List<Integer> myTicket = Arrays.stream(lines[l].split(",")).map(Integer::parseInt).collect(Collectors.toList());
        while (!lines[l].equals("nearby tickets:")) {
            l++;
        }
        List<List<Integer>> validTickets = new ArrayList<>();
        for (int i = l+1; i < lines.length; i++) {
            List<Integer> ticket = Arrays.stream(lines[i].split(",")).map(Integer::parseInt).collect(Collectors.toList());
//            System.out.println(ticket);
            boolean valid = true;
            for (Integer n : ticket) {
                if (!isValid(n, setByLower, setByUpper)) {
//                    System.out.println(n + " not valid");
        ...
        List<Set<String>> allValids = new ArrayList<>();
        for (int i = 0; i < validTickets.get(0).size(); i++) {
            Set<String> allValidFields = null;
            for (List<Integer> ticket : validTickets) {
                Set<Field> validFields = validFields(ticket.get(i), setByLower, setByUpper);
//                if (i == 13) {
//                    System.out.println(String.format("i=%s, num=%s, valids=%s", i, ticket.get(i), validFields));
//                    System.out.println(allValidFields);
//                }
                if (allValidFields == null) {
                    allValidFields = validFields.stream().map(s -> s.name).collect(Collectors.toSet());
                } else {
                    allValidFields = Sets.intersection(allValidFields, validFields.stream().map(s -> s.name).collect(Collectors.toSet()));
                }
            }
            if (allValidFields.isEmpty()) {
                throw new RuntimeException("No valid fields.");
            }
            allValids.add(allValidFields);
        }
        Map<Integer, String> fields = new HashMap<>();
        while (fields.size() != validTickets.get(0).size()) {
            for (int i = 0; i < allValids.size(); i++) {
                Set<String> curValidSet = new HashSet<>(allValids.get(i));
                curValidSet.retainAll(fieldNames);
                if (curValidSet.size() == 1) {
                    String f = curValidSet.stream().findFirst().get();
                    fields.put(i, f);
                    fieldNames.remove(f);
                }
            }
        }
//        System.out.println(fieldToValidPosMap);
        Multiset<String> counts = HashMultiset.create();
        for (Set<String> a : allValids) {
            for (String f : a) {
                counts.add(f);
            }
        }
        System.out.println(Multisets.copyHighestCountFirst(counts));

//        Collections.sort(allValids, new Comparator<Set<String>>() {
//            @Override
//            public int compare(Set<String> s1, Set<String> s2) {
//                return s1.size()-s2.size();
//            }
//        });
//        System.out.println(allValids.get(0).size());
//        System.out.println(allValids.stream().map(s -> s.size()).collect(Collectors.toList()));
//        boolean res = findCombs(allValids, fieldNames, new ArrayList<>());
//        if (res) {
//            System.out.println(validComb);
//        } else {
//            System.out.println("Failed to find valid comb");
//        }

        System.out.println(fields);
        long res = 1;
        for (int i = 0; i < fields.size(); i++) {
            if (fields.get(i).contains("departure")) {
                res = res * myTicket.get(i);
            }
        }
        System.out.println(res);

//        TreeSet<Field> setByLower2 = new TreeSet<>(Comparator.comparingInt((Field f) -> f.lower).thenComparingInt(f -> f.upper));
//        TreeSet<Field> setByUpper2 = new TreeSet<>(Comparator.comparingInt((Field f) -> f.upper).thenComparingInt(f -> f.lower));
//        setByLower2.add(new Field("a", 1, 5));
//        setByUpper2.add(new Field("a", 1, 5));
//        setByLower2.add(new Field("a", 10, 50));
//        setByUpper2.add(new Field("a", 10, 50));
//        System.out.println(validFields(10, setByLower2, setByUpper2));
    }

    private static boolean isValid(Integer n, TreeSet<Field> setByLower, TreeSet<Field> setByUpper) {
        Set<Field> lowerSet = setByLower.headSet(new Field("", n, Integer.MAX_VALUE), true);
        Set<Field> upperSet = setByUpper.tailSet(new Field("", Integer.MIN_VALUE, n), true);
        return !Sets.intersection(lowerSet, upperSet).isEmpty();
    }

    private static Set<Field> validFields(Integer n, TreeSet<Field> setByLower, TreeSet<Field> setByUpper) {
        Set<Field> lowerSet = setByLower.headSet(new Field("", n, Integer.MAX_VALUE), true);
        Set<Field> upperSet = setByUpper.tailSet(new Field("", Integer.MIN_VALUE, n), true);
        return Sets.intersection(lowerSet, upperSet);
    }

    private static List<String> validComb = null;
    private static boolean findCombs(List<Set<String>> valids, Set<String> fieldSet, List<String> curComb) {
        System.out.println("curComb:" + curComb);
        if (valids.size() == curComb.size()) {
            validComb = curComb;
            return true;
        }
        for (String f : valids.get(curComb.size())) {
            if (fieldSet.contains(f)) {
                fieldSet.remove(f);
                curComb.add(f);
                boolean res = findCombs(valids, fieldSet, curComb);
                if (res) {
                    return true;
                }
                fieldSet.add(f);
