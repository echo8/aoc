...
public class Day18Part2 {
    ...
    private static final String INPUT2 = "1 + 2 * 3 + 4 * 5 + 6\n" +
            "1 + (2 * 3) + (4 * (5 + 6))\n" +
            "2 * 3 + (4 * 5)\n" +
            "5 + (8 * 3 + 9 + 3 * 4 * 3)\n" +
            "5 * 9 * (7 * 3 * 3 + 9 * 3 + (8 + 6 * 4))\n" +
...
//            System.out.println(evaluate(tokens));
    ...
    private static class EvalState {
        Long left;
        Token operator;
        Stack<EvalState> multStack;

        public EvalState(Long left, Token operator, Stack<EvalState> multStack) {
            this.left = left;
            this.operator = operator;
            this.multStack = multStack;
        }
...
1 + 2 * 3 + 4 * 5 + 6
  ...
  3   *   7   *  11
    ...
    private static Long evaluate(List<Token> tokens) {
        Stack<EvalState> parenStack = new Stack<>();
        Stack<EvalState> multStack = new Stack<>();
        Long left = null;
        Token operator = null;
        for (Token token : tokens) {
            if (token.type == TokenType.OPEN_PAREN) {
                parenStack.push(new EvalState(left, operator, multStack));
                left = null;
                operator = null;
                multStack = new Stack<>();
                ...
                operator = token;
                multStack.push(new EvalState(left, operator, null));
                left = null;
                operator = null;
                ...
                while (!multStack.isEmpty()) {
                    EvalState multState = multStack.pop();
                    left = left * multState.left;
                }
                EvalState state = parenStack.pop();
                if (state.left != null) {
                    left = left + state.left;
                    operator = null;
                }
                multStack = state.multStack;
            }
//            System.out.println(String.format("After token=%s, left=%s, op=%s, parenState=%s, multState=%s",
//                    token, left, operator, parenStack, multStack));
        }
        while (!multStack.isEmpty()) {
            EvalState multState = multStack.pop();
            left = left * multState.left;
